...
class EncoderHier(nn.Module):
    ...
    def save(self, name, epoch, lam, D):
        fpath = self.fpath_from_name(name, epoch, lam, D)
        makedirpath(fpath)
        torch.save(self.state_dict(), fpath)

    def load(self, name, epoch, lam, D):
        fpath = self.fpath_from_name(name, epoch, lam, D)
        self.load_state_dict(torch.load(fpath))

    @staticmethod
    def fpath_from_name(name, epoch, lam, D):
        return f'ckpts/{name}/{epoch}_{lam}_{D}_enchier.pkl'
