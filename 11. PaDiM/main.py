...
def main():
    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        if not os.path.exists(train_feature_filepath):
            ...
            # Embedding concat
            embedding_vectors = train_outputs['layer1'] # torch.Tensor, (N, 64, 56, 56)

            for layer_name in ['layer2', 'layer3']: 
                embedding_vectors = embedding_concat(embedding_vectors, train_outputs[layer_name])

            #print("Final embedding: ", embedding_vectors.shape) # (N, 448, 56, 56) > 64+128+256 
            ...
            embedding_vectors = embedding_vectors.view(B, C, H * W) # (N, 100, 56*56)
            mean = torch.mean(embedding_vectors, dim=0).numpy() # (100, 56*56)
            cov = torch.zeros(C, C, H * W).numpy() # (100, 100, 56*56)
            I = np.identity(C)
            for i in range(H * W):
        ...
        img_roc_auc = roc_auc_score(gt_list, img_scores) # auroc
